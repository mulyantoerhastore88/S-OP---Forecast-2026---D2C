...
class GSheetConnector:
    ...
    def get_sheet_data(self, sheet_name):
        try:
            ...
            # FIX: Gunakan UNFORMATTED_VALUE untuk numeric data
...
def clean_currency(val):
    """Clean currency values (for Rupiah)"""
    if pd.isna(val) or val == '':
        return 0
    
    val_str = str(val).strip()
    
    # Handle kasus sudah numeric
    if isinstance(val, (int, float)):
        return float(val)
    
    # Hapus 'Rp' dan karakter currency lainnya
...
def clean_stock_qty(val):
    """Clean stock quantity values"""
    if pd.isna(val) or val == '':
        return 0
    ...
    # Coba ekstrak angka dengan regex yang lebih akurat
...
def find_matching_column(target_month, available_columns):
    """Find matching column name for target month"""
...
# ============================================================================
# 2. DATA LOADER (UPDATED - FIXED STOCK ISSUE)
# ============================================================================
...
def load_data_v5(start_date_str):
    try:
        ...
        # Load data dengan format yang tepat
        ...
        if l3m_cols:
            sales_df['L3M_Avg'] = sales_df[l3m_cols].replace('', 0).astype(str).applymap(clean_stock_qty).mean(axis=1).round(0)
        else:
            sales_df['L3M_Avg'] = 0
        ...
        # Clean ROFO month values
        for m in horizon_months:
            if m not in merged_df.columns:
                merged_df[m] = 0
            else:
                merged_df[m] = merged_df[m].apply(clean_stock_qty)

        # FIXED: Stock Quantity - gunakan clean_stock_qty khusus
        if not stock_df.empty and 'sku_code' in stock_df.columns:
            ...
            # Debug logging untuk stock
            debug_sample = stock_df[['sku_code', stock_col]].head(3)
            st.session_state.stock_debug = debug_sample
            ...
            # FIX: Gunakan clean_stock_qty bukan clean_currency
            merged_df['Stock_Qty'] = merged_df['Stock_Qty'].apply(clean_stock_qty)
        else:
...
def calculate_pct(df, months):
    """Calculate percentage vs L3M Average"""
    df_calc = df.copy()
    for m in months:
...
with st.sidebar:
    ...
    selected_start_str = st.selectbox("Forecast Start Month", 
    ...
    with st.expander("🕵️ Debugger"):
        if 'missing_months' in st.session_state and st.session_state.missing_months:
            ...
            st.success("All 12-Month Columns Found/Mapped!")
        
        # Show stock debug info
        if 'stock_debug' in st.session_state:
            st.write("Stock Debug Sample:")
            st.dataframe(st.session_state.stock_debug, use_container_width=True)
...
# Show sample of stock data for verification
with st.expander("🔍 Data Verification", expanded=False):
    if 'Stock_Qty' in all_df.columns:
        st.write("Sample Stock Quantities (First 10 rows):")
        sample_data = all_df[['sku_code', 'Product_Name', 'Stock_Qty']].head(10)
        st.dataframe(sample_data, use_container_width=True)
        
        # Summary stats
        st.write("Stock Quantity Summary:")
        st.write(f"Total SKUs: {len(all_df)}")
        st.write(f"Average Stock: {all_df['Stock_Qty'].mean():.1f}")
        st.write(f"Min Stock: {all_df['Stock_Qty'].min():.1f}")
        st.write(f"Max Stock: {all_df['Stock_Qty'].max():.1f}")
...
with tab1:
    ...
    hist_cols = [c for c in edit_df.columns if '-' in c and c not in st.session_state.horizon_months 
                 and 'Cons' not in c and '%' not in c][-3:]
    ...
    js_cover = JsCode("""
        function(p) { 
            if(p.value > 1.5) return {'backgroundColor': '#FCE7F3', 'color': '#BE185D', 'fontWeight': 'bold'}; 
            return null; 
        }
    """)
    ...
    js_pct = JsCode("""
        function(p) { 
            if(p.value < 90) return {'backgroundColor': '#FFEDD5', 'color': '#9A3412', 'fontWeight': 'bold'}; 
            if(p.value > 130) return {'backgroundColor': '#FEE2E2', 'color': '#991B1B', 'fontWeight': 'bold'}; 
            return {'color': '#374151'}; 
        }
    """)
    ...
    for c in ag_cols:
        if c not in ['sku_code', 'Product_Name', 'Channel', 'Brand', 'SKU_Tier', 
                     'Month_Cover', 'Product_Focus', 'floor_price'] and '%' not in c:
            gb.configure_column(c, type=["numericColumn"], valueFormatter="x.toLocaleString()", minWidth=105)
    
    # Configure percentage and consensus columns
    for m in cycle_months:
        if f'{m}_%' in ag_cols:
            gb.configure_column(f'{m}_%', header_name=f"{m} %", 
                               type=["numericColumn"], 
                               valueFormatter="x.toFixed(1) + '%'", 
                               cellStyle=js_pct, minWidth=90)
        if f'Cons_{m}' in ag_cols:
            gb.configure_column(f'Cons_{m}', header_name=f"✏️ {m}", 
                               editable=True, cellStyle=js_edit, width=115, 
                               pinned="right", type=["numericColumn"], 
    ...
    grid_res = AgGrid(ag_df, gridOptions=gb.build(), allow_unsafe_jscode=True, 
                     update_mode=GridUpdateMode.VALUE_CHANGED, height=600, 
                     theme='alpine', key='v5_worksheet', use_container_width=True)
        ...
        if st.button("☁️ Push (GSheets)", type="secondary", use_container_width=True):
            if 'edited_v5' not in st.session_state:
                ...
                with st.spinner("Pushing to Google Sheets..."):
                    keep_cols = ['sku_code', 'Product_Name', 'Channel', 'Brand', 
                                'SKU_Tier', 'Product_Focus'] + [f'Cons_{m}' for m in cycle_months]
                    final_df = st.session_state.edited_v5[keep_cols].copy()
                    final_df['Last_Update'] = datetime.now().strftime('%Y-%m-%d %H:%M')
                    
                    gs = GSheetConnector()
                    ok, msg = gs.save_data(final_df, "consensus_rofo")
    ...
    with c_info:
        ...
        for m in cycle_months:
            if f'Cons_{m}' in updated_df.columns:
                total_consensus += updated_df[f'Cons_{m}'].sum()
        st.metric("Total Consensus (M1-M3)", f"{total_consensus:,.0f}")
...
with tab2:
    ...
    with c_view:
        chart_view = st.radio("Chart View:", ["Total Volume", "Breakdown by Brand"], 
                             horizontal=True)
    with c_year:
    ...
    with stylable_container(key="kpi_v5", css_styles="""
        {background-color:#F1F5F9; padding:20px; border-radius:10px; border:1px solid #CBD5E1;}
    """):
        k1, k2 = st.columns(2)
        period_label = "2026 Only" if show_2026_only else "12-Month"
        
        with k1:
            st.metric(f"{period_label} Volume", 
                     f"{grand_total_qty:,.0f} pcs", 
                     "Forecast")
        
        with k2:
            st.metric(f"{period_label} Revenue", 
                     f"Rp {grand_total_val/1_000_000_000:,.2f} M", 
...
# ============================================================================
st.markdown("---")
st.caption("ERHA S&OP Dashboard ")
