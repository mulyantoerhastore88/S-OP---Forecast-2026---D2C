...
# ============================================================================
st.set_page_config(
    page_title="ERHA S&OP Dashboard V5.5 (Fixed Stock)",
    page_icon="📊",
    layout="wide",
...
class GSheetConnector:
    ...
    def get_sheet_data(self, sheet_name):
        try:
            ...
            # PENTING: Gunakan UNFORMATTED agar angka desimal terbaca benar
...
def clean_currency(val):
    """
    FIXED: Mencegah penghapusan titik desimal agar 12402.0 tidak jadi 124020
    """
    if pd.isna(val) or val == '': return 0
    
    # Jika sudah angka, langsung kembalikan (Safety)
...
def load_data_v5(start_date_str):
    try:
        ...
        if l3m_cols:
            # Terapkan clean_currency juga di sini untuk keamanan
            for col in l3m_cols:
                sales_df[col] = sales_df[col].apply(clean_currency)
        ...
        if not stock_df.empty and 'sku_code' in stock_df.columns:
            ...
            # Bersihkan dan Aggregasi jika ada duplikat SKU
            stock_df[stock_col] = stock_df[stock_col].apply(clean_currency)
            stock_df_agg = stock_df.groupby('sku_code')[stock_col].sum().reset_index()
            stock_df_agg.rename(columns={stock_col: 'Stock_Qty'}, inplace=True)
        ...
        # Metrics
        # Gunakan denominator 0.1 jika L3M = 0 untuk menghindari error, tapi idealnya dihandle di JS
